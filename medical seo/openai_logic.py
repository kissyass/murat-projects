...
def generate_content(content_data, client):
    """Generate SEO metadata using OpenAI."""
    try:
        company = content_data['company']
        add_info = content_data['add_info']
        location = content_data['location']

        prompt = f"""Could you please find me information about this company: {company}, {add_info}, {location} on turkish language 
            and create me an overview of this company. Include what they do (services), address, contact info, workish hours, any information 
            about their facilities. no additional messages, if you cant find info, then skip. But please try to find info cause you can.
            """
...
def generate_seo_metadata(seometa_data, client):
    ...
    try:
        ...
        meta_prompt = f"""Could you please write me a Focus Keyword, SEO Title, Meta Description and URL Slug in this format 
                ...
                the topic. The main first focus kw should be 100% url slug. Separated by the comma add 4 more focus kws that will appear in the text often. 
...
def generate_article(article_data, client):
    ...
    try:
        ...
        article_prompt = f"""Could you please write me a company description for 4000 words length. Company description text has to be formatted in HTML 
                    ...
                    include heading h1, h2, h3 and h4. Focus Keyword has to appear at least 30 times in the content description text. Preferably every 4th or 5th sentence 
                    should contain focus keyword. Only give me the content description, no need for any additional messages. Also add why customers will want
                    and should by this content from us, write as if it is what they really need and we can sell it or something like this please. 
                    The Topic for the article: {topic}
                    The Services they do and all contact and general inforamtion: {content}
                    Location of the place: {location}
                    Inlcude this info as we are promoting thei business and dont forget to mention the place.
                    Additional SEO information for the content description:
                    {seo_metadata}
...
def generate_data(content, client):
    """Generate SEO metadata using OpenAI."""
    try:

        prompt = f"""Could you please find me this places phone number, website, email, full address, latitude and longitude, working hours, 
            and youtube video about this company {content}? if information is not provided, please feel free to search online. 
            The format of the output must be excatly like this without any additional messages:
            phone number:
            website:
            email:
            address:
            ...
            For those fields you couldnt find info just write NO INFO, but please try to find online first the information. No additional messages. 
            """
        completion = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
        ...
        try:
            web = meta_data['website'].split('(')
            meta_data['website'] = web[1].replace(')', '')
        except Exception as e:
            print(e)
        print(meta_data)
        return meta_data
...
def generate_categories(content, client):
    """Generate SEO metadata using OpenAI."""
    df = pd.read_csv("categories.csv")
    categories = df[['id', 'Title', 'link']].to_dict(orient='records')

    try:

        prompt = f"""Could you please find me 1 category from here {categories} that 100% describes this company {content}.
            No additional messages. As an output format, here what you need to follow 100%:
            
            category_id: id of the selected category from the list.
            image: link from mother category from the list.
            """
        completion = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
...
def generate_amenties(content, client):
    """Generate SEO metadata using OpenAI."""
    try:
        amenties = pd.read_csv('amenties.csv')

        prompt = f"""Could you please find me amenties from here {amenties} that 100% are in this company {content}. Use only those from the list. 
            No additional messages. As an output format, here what you need to follow 100%:
            
            amenty_ids: ids of the selected amenty from the list separated by comma.
            """
        completion = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
