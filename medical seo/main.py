...
from openai_logic import generate_html_info, generate_article_summary, generate_content, generate_data, generate_categories, generate_amenties
...
from database import insert_summary_into_table, get_all_summaries_from_table, insert_account_post_log, export_post_logs_to_csv, export_post_logs_to_excel
...
class App:
    ...
    def show_data_management_frame(self):
        ...
        if self.current_record_table is None:
            ...
            tk.Button(frame, text="Export Post Logs", width=20, command=self.export_post_logs_ui).pack(pady=10)
    ...
    def upload_excel_file(self):
        ...
        if file_path:
            ...
            try:
                ...
                required_columns = ["Title", "ülke", "şehir", "Additional_Info"]
    ...
    def start_generation(self):
        ...
        while self.generating:
            ...
            record_id, title, country, city, additional_info = record
            ...
            try:
                content_data = {"company": topic, "location": location, "add_info": additional_info}
                content = generate_content(content_data, self.client)
                self.progress_text.insert(tk.END, "Content is generated.\n")
            except Exception as e:
                self.progress_text.insert(tk.END, f"Error generating Content: {e}\n")
                continue

            # Generate SEO metadata.
            try:
                meta_data = {"year": current_year, "topic": topic, "content": content, "location": location}
            ...
            try:
                company_info = generate_data(content, self.client)
            except Exception as e:
                company_info = {}
                print("error generation company info for table")
            
            try:
                content_category = generate_categories(content, self.client)
            except Exception as e:
                content_category = {}
                print('error generating category')

            try:
                amenties = generate_amenties(content, self.client)
            except Exception as e:
                amenties = {}
                print('error generating amenties')
            ...
            try:
                post = self.post_article_to_wordpress(final_article, seo_metadata, topic, tag_ids, uploaded_images)
                company_info['country'] = country
                company_info['city'] = city
                company_info['post_url'] = post["post_url"]
                company_info['post_id'] = post["post_id"]
                company_info['category'] = content_category["category_id"]
                company_info['category_image'] = content_category["image"]
                company_info['amenities'] = amenties["amenty_ids"]
                insert_account_post_log(self.wp_base_url, company_info)
                self.progress_text.insert(tk.END, f"Article posted for record {record_id}: {post['post_url']}\n")
            except Exception as e:
    ...
    def post_article_to_wordpress(self, article_html, seo_metadata, topic, tag_ids, uploaded_images):
        try:
            ...
            update_response = requests.post(f"{self.wp_base_url}/wp-json/wp/v2/job-listings", json=post_data, auth=self.auth)

            if update_response.status_code == 201:
                post_url = update_response.json().get("link", "none")
                record_id = update_response.json()["id"]
                post = {"post_id": record_id, "post_url": post_url}
    ...
    def get_term_id(self, slug, taxonomy):
        try:
            response = requests.get(
                f"{self.wp_base_url}/wp-json/wp/v2/{taxonomy}",
                params={"slug": slug},
                auth=self.auth
            )
            if response.status_code == 200 and response.json():
                ...
                print(f"Term '{slug}' not found in taxonomy '{taxonomy}': {response.text}")
        except Exception as e:
            print(f"Error retrieving term ID for {slug} in {taxonomy}: {e}")
        return None
    ...
    def export_post_logs_ui(self):
        file_path = filedialog.asksaveasfilename(
            defaultextension=".csv",
            filetypes=[("CSV files", "*.csv")],
            title="Save Post Logs"
        )
        if not file_path:
            return
        try:
            export_post_logs_to_csv(self.wp_base_url, file_path)
            messagebox.showinfo("Success", f"Post logs exported to:\n{file_path}")
        except Exception as e:
